                ...
                if rse['name'] in rses:
                    ...
                    for protocol in rse['protocols']:
                        ...
                        if o.scheme == 'srm':
                            ...
                            # @ADCR_ADG.CERN.CH
                            subquery = '''select  hextoraw('%(rse_id)s') as rse_id, substr(sfn, instr(sfn, '/', -1) + 1) as name, 1 as is_migrated, sfn, null as path from cns_file_replica@DBFTSBNL.BNL.GOV where (sfn like '%(short_surl)s'||'%%' OR sfn like '%(long_surl)s'||'%%') ''' % locals()
                            if rse['is_tape']:
                                subquery = '''select hextoraw('%(rse_id)s') as rse_id, case when INSTR(substr(sfn, instr(sfn, '/', -1) + 1),'__DQ2')> 0 then SUBSTR(substr(sfn, instr(sfn, '/', -1) + 1), 1, INSTR(substr(sfn, instr(sfn, '/', -1) + 1), '__DQ2')-1)  else substr(sfn, instr(sfn, '/', -1) + 1) end as name, 1 as is_migrated, sfn, replace(replace(sfn, '%(long_surl)s'), '%(short_surl)s')  as path from  cns_file_replica@DBFTSBNL.BNL.GOV   where (sfn like '%(short_surl)s'||'%%' OR sfn like '%(long_surl)s'||'%%') ''' % locals()
