...
if __name__ == '__main__':
    ...
    rses = [u'LUCILLE_DATADISK', u'LUCILLE_LOCALGROUPDISK', u'LUCILLE_PRODDISK', u'LUCILLE_SCRATCHDISK', u'LUCILLE_USERDISK']
    try:
                ...
                if rse['name'] in rses:
                    ...
                    for protocol in rse['protocols']:
                        ...
                        if o.scheme == 'srm':
                            ...
                            # @ADCR_ADG.CERN.CH
                            subquery = '''select  hextoraw('%(rse_id)s') as rse_id, substr(sfn, instr(sfn, '/', -1) + 1) as name, 1 as is_migrated, sfn, null as path from cns_file_replica@ATLFCREP.BNL.GOV  where (sfn like '%(short_surl)s'||'%%' OR sfn like '%(long_surl)s'||'%%') ''' % locals()
                            if rse['is_tape']:
